...
import { SectionList, Text, View, StyleSheet, Image, TouchableOpacity, ScrollView, TextInput } from "react-native"
...
function Tooltip({ x, y }: { x: SharedValue<number>; y: SharedValue<number> }) {
  return <Circle cx={x} cy={y} r={5} color={Colors.primary} />
}
...
const DetailsPage = () => {
  ...
  const { data: tickers } = useQuery({
    queryKey: ['tickers'],
    queryFn: async (): Promise<Ticker[]> => fetch(`/api/tickers`).then((res) => res.json()),
  });
